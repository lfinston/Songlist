...
int write_to_toc_sub_file(ofstream *toc_file_ptr, vector<Song>::iterator &iter, int two_or_three_digits);
...
/* ** (2)  */
   ...
   sub_filecards_file << "%% sub_filecards.tex" << endl
                      << "%% Generated by `songlist' " << datestamp << endl << endl
                      << "\\begingroup" << endl << "\\catcode`\\%=\\other" << endl << endl;
...
/* **** (4) */
       ...
       cerr << "iter->title       == " << iter->title << endl 
            ...
            << "iter->entry_day   == " << iter->entry_day << endl
               ...
               && iter->entry_day > filecard_day))
       {
          ...
          if (new_filecards_first_time == true)
          {
              ...
              new_filecards_first_time = false;

              new_filecards_file.open("newfilecards.tex");

              new_filecards_file << "%% newfilecards.tex" << endl
                                 << "%% Generated by `songlist' " << datestamp 
                                 << endl << endl
                                 << "\\FilecardSection{Songs and productions added since " 
                                 << filecard_year << "." << filecard_month 
                                 << "." << filecard_day << ".}" << endl;
          }  /* |if|  */
...
// getchar(); 
       ...
       if (new_filecard_ctr == 8)
       {
         new_filecards_file << "\\eject" << endl << endl;
         new_filecard_ctr = 0;
       }
...
/* **** (4) */
         ...
         write_to_toc_sub_file(toc_file_ptr, iter, 3);
...
/* **** (4) */
             ...
             || temp_char <= 'h')
         {
             ...
             write_to_toc_sub_file(toc_file_ptr, iter, 2);
...
/* *** (3)  */
   ...
   if (!new_filecards_first_time)
      new_filecards_file << endl << endl << "\\endinput" << endl << endl;
